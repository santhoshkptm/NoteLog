...
- Fixed About page content drawing under the status-bar/notch area.
- Added a first-run `NoteLog Guide.txt` to the selected folder with usage, storage, backup and sync guidance.
- Added a sketch-style NoteLog launcher icon (notepad, clock, text lines and pencil-stroke background).
